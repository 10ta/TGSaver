...
一条消息里贴多个推文链接时，会尽量拼成**一条** Telegram 消息返回：
媒体合成一个相册，每条推文的昵称和原帖链接后面带上它的媒体在相册里的位置。
...
- **必然走 `media` 逻辑。** 一条消息只能有一个链接预览，装不下多条推文的媒体，
  所以合并时不查预览
- **超过 10 个媒体就切一刀**（Telegram 相册上限）。一条推文的媒体不会被拆散；
  剩下的重新入队，还是多条就继续合并，只剩一条就走单条的 auto 流程
- **纯文字推文**没有媒体位置，就不带序号，照样参与合并
- **取不到的跳过**，末尾注明「其中 N 条未能取到」；全部取不到才算失败
- **说明超过 1024 字时截断正文**，短的原样保留，只削长的——一条长推不会
  把别人的正文全挤掉。条目太多以致正文削到 0 仍装不下时，媒体不带说明，
  说明另发一条文字消息

只发一条链接时行为不变：不带序号，照常走 `TWEET_MODE`。
...
在消息末尾加 `fw` 和去向，结果就不再发给你，而是直接进那个频道 / 群 / 私聊：

```
https://x.com/a/status/123 fw mychannel
https://t.me/chan/456 fw -1001234567890
https://x.com/a/status/123 fw            ← 用上次的去向
fw @mychannel                            ← 只设默认去向并校验权限
```

`FW` 大写也认。去向支持 `optv4`、`@optv4`、数字 id、`t.me` 链接。
可以和 `nosp` 并用，`fw nosp` 表示用上次的去向并去掉剧透。

不带 `@` 的用户名按 Telegram 规则识别：字母开头、5 到 32 位。`fw` 后面
跟了看不懂的东西时会明确提示「这次不转发」，链接照常处理——不会出现以为
转发了、其实什么都没发生的情况。

**fw 是附加步骤，不改变现有流程。** 你照常收到那一份，之后再额外转一份
到去向。转发失败只提示原因，已经送达的那份不受影响。

**怎么做到不露出 bot。** 用 user 账号的
`forward_messages(drop_author=True)`，也就是客户端上「hide sender name」
那个选项。消息先由 **user 账号**生成在中转频道里，再从那儿转出去：

- 服务端操作，不花流量，没有「转发自」抬头
- 转到频道显示频道自己的署名，转到群显示你的账号，都看不到 bot
- 中转频道那份自然留作归档

bot 发的东西转不走（一转就露出 bot），所以转发的源头只能是中转频道。
走服务器中转的内容那里本来就有副本，直接转即可；快通道下 bot 是直接发给
你的、中转频道没有副本，这时由 user 账号另做一份：预览用
`invert_media` + `InputMediaWebPage` 发出，媒体把 URL 交给 Telegram
服务器自己拉（本机零流量），拉不动才回退到本机下载上传。

**去向的记忆**：按用户各自存在库里，重启后仍然有效。第一次用 `fw` 必须
指定去向。换去向时会当场校验一次（能不能解析到、有没有发布权限），
不会等任务跑完才发现发不出去。

转发用的是机主的账号（和取消息同一套凭据），所以授权用户用 `fw` 时，
目标那边看到的是机主账号 / 频道的署名。
...
- **多条合并**：序号与区间、纯文字条目、跳过失败条目、按媒体数切分与递归入队、
  正文预算的公平分配、固定部分超长时的退路、合并不查预览、回退后不重复取数
- **转发**：指令解析（普通句子里的 "fw" 不误伤、裸用户名、`fw nosp` 组合、
  看不懂的去向要提示而不是静默忽略）、去向规整与拒绝非法输入、
  权限校验、隐藏来源、fw 是附加步骤（本人那份不能少）、转发失败不影响
  已送达的内容、凭据检查看的是凭据归属者而不是发起人、Telegram 拉不动时的回退
- **user 账号发送**：用**真实的 Telethon 客户端**、只替换最底层网络发送来测，
