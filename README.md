...
因为 bot 发的东西转不走（一转就露出 bot），带 `fw` 时整条链路不经过 bot：
推文的预览由 user 账号用 `invert_media` + `InputMediaWebPage` 发出，
媒体则把 URL 交给 Telegram 服务器自己拉（本机零流量），拉不动才回退到
本机下载上传。
...
**注意**：成功时不会有任何回复——你自己不收内容，结果直接进了去向。
失败时才会告诉你原因。
...
- **转发**：指令解析（普通句子里的 "fw" 不误伤）、去向规整与拒绝非法输入、
  权限校验、隐藏来源、带 fw 时 bot 完全不参与投递、Telegram 拉不动时的回退
